...
Live model evaluations live in `src/evals/` and run together from the repository root with `yarn test:evals`. The Generate Emoji evaluation checks the ranking: each semantic category (from issue #4400, with Events spelled Party and Irritable omitted) lists two or three must-have concepts with their acceptable variants, and every concept must appear in the generated top five. Each category is generated three times and must satisfy the criterion in a majority, which replaces the retry for these cases (`retry: 0`) so that a single lucky attempt cannot pass a category; the generated lists are logged for every category so the fixtures can be tuned against them. The Generate Thought evaluation asserts against the model's complete replacement thought, covering alphabetical sequence completion, typo correction, parent and sibling context, nested capital generation, top-level sequences, partial fill-in, replacement of misplaced items, and omission of context-only text. The Organize Thought evaluation asserts directly against the model's indented textual final state, covering semantic groups, compound splitting without duplication, preserved nesting, reclassification into existing categories, and omission of context-only thoughts. The Generate Thought and Organize Thought cases retry failures up to twice to accommodate model nondeterminism. Evaluations require the corresponding service key or the shared fallback in `.env.local` and are intentionally excluded from the default deterministic test suite.

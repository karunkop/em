...
BrowserStack, Vercel Preview, and Evals are the exception on both counts. They trigger on `pull_request_target` and check out `github.event.pull_request.head.sha`, so they test the head commit rather than a merge with `main` — and they keep running while a pull request conflicts, which is why a conflicting pull request reports those checks and none of the others.
...
Agent Scripts and Evals run the other way round, on a `paths` allowlist: each names the files that are its inputs and runs only when one of them changes. The consequence is the same — an unmatched pull request reports no check — so neither may be made required either. Evals additionally selects which evaluation to run from the changed files, so a Generate Emoji prompt change is never failed by a Generate Thought flake; a change to what every prompt shares (`completeChat.ts`, the `@types` enums) runs them all.
...
BrowserStack and Evals supersede too, but key their groups on `github.event.pull_request.number` because they are `pull_request_target` workflows (see above). TDD's iOS job runs against the same BrowserStack account from a *different* group, so it contends for the shared session cap either way — both wait for a free slot in-process rather than for each other (see [Layered BrowserStack concurrency](#layered-browserstack-concurrency)).
...
| **Evals** | [`.github/workflows/evals.yml`](../.github/workflows/evals.yml) | `yarn test:evals` against the live OpenAI API, for the evaluations whose prompt or inputs the pull request changed. | Uses `pull_request_target` so the `OPENAI_API_KEY` repository secret (a dedicated, budgeted key) is available on fork pull requests, runs only on a `paths` allowlist of `packages/ai/src/{prompts,evals,completeChat.ts,@types}`, and deduplicates per PR. Nondeterministic by nature: a failed category is a signal to rerun and read, not proof of a regression. `workflow_dispatch` runs every evaluation against a branch. |
